...
ADR_RULES = [
    ...
    CustomRuleDef(
        ...
        description=(
            ...
            "that at least one populated parameter matches that "
            "prefix. Item types without a known mapping fall back to "
            "the original check: any populated design parameter is "
            "sufficient."
        ),
        notes=(
            ...
            "ADR_DIM_ESTIMATEDESIGNDETAILS is 1:N on ROW_ID (about "
            "10 parameters per item), so the data product builder "
            "derives ``DESIGN_KEY_PARAMETER_NAMES``: the pipe-"
            "separated list of parameter names whose "
            "``DESIGN_PARAMETER_VALUE`` is populated, one list per "
            "item. For each ``ITEM_TYPE`` in the "
            "``_A5_KEY_DESIGN_PREFIX`` mapping (33 types covering "
            "major equipment, piping, structural, civil, electrical "
            "and instrument categories), the rule checks that at "
            "least one entry of that list starts with the expected "
            "COA prefix. The starts-with match handles composite "
            "prefixes such as ``314.0,315.2,316.0-Diameter-Section-1`` "
            "for vertical pressure vessels. For example, "
            "``EstimatePump`` requires a ``324.0-*`` parameter (Pump "
            "Type, Pump Material, Driver Power Rating); "
            "``EstimateShellAndTubeExchanger`` requires ``311.1-*`` "
            "(TEMA Type, Surface Area Per Shell); "
            "``EstimateAbovegroundInstrumentPiping`` requires "
            ...
            "any-populated-parameter check. Items with no non-zero "
