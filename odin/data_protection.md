...
**Status:** Applicable
**Comment:** The app caches data products, scorecards, and reference datasets in `session_state`, plus a process-wide shared Snowflake connection (`get_shared_client`). Purge controls exist: `_clear_workflow_state_for_domain_switch` / `restart_app` clear workflow state and call `close_shared_client()` on restart and on any domain/mode switch (`src/snowflake_client.py:210-221`). Gap: within an active session the cached data persists unencrypted in RAM until restart/exit. Control is partial (purge-on-restart, not continuous protection); acceptable for a local single-user model but should be stated honestly.
