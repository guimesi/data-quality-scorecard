...
- **Review pinned versions for advisories** regardless of channel. ✅ DONE (dev/CI): deps were
  refreshed on PyPI (e.g. `urllib3` 1.26→2.x, streamlit→1.58, requests→2.34, PyJWT→2.13,
  pyarrow/pillow/python-dotenv/pytest); `pip-audit` on `requirements.lock` reported no known
  vulnerabilities as of the refresh (point-in-time — **JFrog Xray on the PR is authoritative**).
  **SiS prod caveat:** the Snowflake Anaconda channel caps **streamlit at 1.52.2** (<1.54), so
  Xray on `environment.yml` will flag two streamlit advisories that are **accepted risks**:
  GHSA-7p48-42j8-8846 (SSRF/NTLM, Windows-only; SiS is Linux → N/A) and PYSEC-2026-212
  (cache weak-hash, local access → negligible). The app uses no streamlit API above ~1.27, so
  1.52.2 is functionally sufficient; re-pin when the channel offers a newer build.
...
- **`pyproject.toml` / ruff `target-version`** and **CI `python-version`** ✅ DONE: reconciled to
  **py311** across the board (ruff `target-version = "py311"`, CI `python-version: 3.11`,
  `environment.yml` `python=3.11`) so lint/test match the SiS runtime.
...
- [~] P1 Review pinned deps for advisories — dev/CI refreshed (urllib3→2.x, streamlit 1.58, etc.; pip-audit clean as of refresh). **SiS prod streamlit capped at 1.52.2 by Anaconda channel — 2 advisories <1.54 accepted as non-material (see §2)**
...
- **Re-pin streamlit above 1.52.2** once the Snowflake Anaconda channel offers a newer build (currently capped at 1.52.2; the two <1.54 advisories are accepted as non-material — see §2).
