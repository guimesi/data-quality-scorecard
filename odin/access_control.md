...
**Status:** Applicable
**Comment:** The app is read-only **by design** — it only ever issues `SELECT` (no INSERT/UPDATE/DELETE/DDL). Note the data layer's `fetch_query()` executes whatever SQL its callers pass, so the guarantee of non-mutation *under a compromised session* is not enforced in app code alone; it must be backed at the platform tier. **Follow-up (high priority):** run the SiS app under a **dedicated least-privilege, read-only role** scoped to the required schemas (not a broad personal/admin role) so that write operations are denied by Snowflake regardless of the code path. Cross-ref Access Control Architecture.
