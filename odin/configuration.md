...
**Comment:** The app exposes no custom HTTP endpoints/methods; method handling belongs to Streamlit (local) and Snowflake (SiS). There is no app-layer method configuration to set. Platform-delegated.

**Question:** Strict whitelist for Access-Control-Allow-Origin (CORS); no "null" origins.
**Status:** Not Applicable
**Comment:** The app sets no CORS headers. Streamlit ships CORS/XSRF protections on by default (and no config.toml weakens them); under SiS, CORS is Snowflake-managed. Follow-up: if a `.streamlit/config.toml` is ever added for local use, do not disable `enableCORS`/`enableXsrfProtection`.

**Question:** Authenticate HTTP headers added by trusted proxies/SSO (e.g. bearer tokens).
**Status:** Not Applicable
**Comment:** The app performs no header-based authentication and never trusts inbound headers for identity — auth is the Snowflake session (SSO locally / active session under SiS). There is no proxy/SSO header trust path to secure.
...
**Status:** Applicable
**Comment:** **Substantially addressed.** CI now runs **SCA (pip-audit)** report-only (`security.yml`), and dependencies were **refreshed to clear known advisories** (incl. the previously-flagged `urllib3` 1.26→2.x); `pip-audit` reports **no known vulnerabilities** on `requirements.lock`. **Residual gap:** the in-repo SCA scans the PyPI lockfile as a proxy — the **authoritative production scan must run via JFrog Xray against `environment.yml`** (Anaconda), and automated dependency-update alerting is still to add. Follow-up: JFrog Xray on `environment.yml` + update alerting.
