...
### 📑 Data Quality Report (interactive HTML + PDF)

Step 6 builds **two artefacts from one view model** (`ui/step_06/report/`,
scope: DQRs only) once per run and caches them in the session:

- **Data Quality Report (HTML)** - ONE fully self-contained, interactive
  file (no CDN, fonts or Plotly): sticky nav + search, executive summary,
  and per Data Product the overview (gauge, threshold distribution),
  By CDE → DQRs → By Dimension lists with lazy drill-downs into the
  lowest-scoring failing rows, the Lowest-scoring rows table (every DP
  column + reference-dataset fields + one 100/0 column per DQR), the
  persisted History (trend, run log, what changed / drift) and the exact
  configuration used. Header: Generated (UTC) + Project filter; the
  "About this report" footer carries Generated by + Run identifier.
- **Data Quality Report (HTML + CSS + JS, zip)** - the *same* interactive
  report split into three files (`dq_scorecard_report_<DOMAIN>.html`,
  `.css`, `.js`) that reference each other by relative name, for hosts
  that strip inline `<style>`/`<script>` from `.html` files but serve
  side-car assets untouched (a SharePoint document library). Unzip the
  three files into one folder and link the `.html` (e.g. from an
  Airtable button); the names carry no timestamp so a fixed link keeps
  pointing at the latest upload. The `.js` file carries the JSON payload
  and the unchanged runtime.
- **Data Quality Report (PDF)** - the complete paginated A4 edition
  (cover, executive summary, per DP: overview + CDE × DQR heatmap, CDEs /
  dimensions / what changed, DQR table, DQR detail cards with 5 sample
  failing rows, landscape Lowest-scoring rows sheets (25 rows: values and
  DQR flags), configuration; About). Rendered by headless Chromium
  (Playwright or a Chrome / Edge / Chromium binary via
  `DQS_CHROMIUM_PATH`, `prefer_css_page_size` so the landscape sheets come
  out of one conversion). Without a converter the app offers the
  **print-ready HTML** instead (Ctrl+P in Chrome/Edge gives the same PDF).
...
`ReportArtifacts(html, pdf, pdf_html, metadata, filenames, split)`
contract is what a future SharePoint publisher consumes; the Airtable
push keeps attaching the interactive HTML.

`python scripts/build_sample_report.py` generates every edition from
mock data (`output/dq_report/`, split files under `split/`).
...
│   │   ├── report/             # 📑 Pure report builder: collect -> ReportModel -> HTML + PDF editions
