...
A `TableDef` can override that default per column with `column_aggregations` (any string or callable `groupby().agg` accepts) and prepare row-level inputs with `derive_columns` before the collapse. ADR's design dimension uses both: `_adr_design_derive` keeps each parameter name only when its value is populated, and `join_unique` joins the per-item names into `DESIGN_KEY_PARAMETER_NAMES` (`name1|name2|…`) so A5 can see every parameter of an item instead of just the first row.
