...
| **A5** (ADR) - Key design details present when quantity exists | Consistency | No | `QTY_QUANTITY`, `ITEM_TYPE`, `DESIGN_PARAMETER_NAME`, `DESIGN_PARAMETER_VALUE` | - | Type-aware: items with a non-zero aggregated quantity (`SUM(ADR_FACT_ESTIMATEQTYRESULTS.QUANTITY) <> 0`) whose `ITEM_TYPE` is in the `ITEM_TYPE → COA prefix` mapping (33 types) must carry a `DESIGN_PARAMETER_NAME` starting with the expected prefix (e.g. `EstimatePump` → `324.0-*`; starts-with match so `314.0,315.2,316.0-Diameter-Section-1` resolves) **and** a populated `DESIGN_PARAMETER_VALUE`. Unmapped item types keep the original check (any populated value). Items with no non-zero quantity are out of scope and pass. |
