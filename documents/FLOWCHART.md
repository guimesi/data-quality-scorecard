...
flowchart TD
    ...
    BUCKET --> RENDER["Render dashboard:<br/>gauge, source breakdown,<br/>By CDE / Dimension (Standard +<br/>Custom blended), custom rules tab,<br/>worst rows (with per-rule score<br/>columns + weight in header +<br/>reference-dataset columns)"]
...
- **Step 6**: thresholds for green / yellow / red come from `Settings.score_green_min` and `Settings.score_yellow_min` (configurable in `.env`). Final overall score combines Standard and Custom subscores by their Step-4 source weights.
